...
<div class="alert alert-danger">
  ...
  <?php echo $reason_for_cancellation__reason; ?> </div>
<?php endif ?>
<form method="POST" id="validation" class="form_commerce " style="position:relative;" enctype="multipart/form-data">
...
<div class="box">    <input type="file" class="inputfile inputfile-6" name="catalogue" id="catalogue" accept="application/pdf">
      <label for="catalogue" class="commerce_logo"><span></span> <strong> Choose a file…</strong></label></div>
      <?php if (!empty($catalogue)): ?>


      <div><a href="<?php echo wp_get_attachment_url($catalogue); ?>" class="download_catelog">Download</a></div>
      <?php endif ?>
    </div>
  </div>
...
<br>
<br>
 <div class="row logo_whocommece">
      <div class="col-md-12">
      <h6>Logo (Upload Only JPG, PNG )</h6>
      <div class="box">
        <input type="file" name="logo" id="logo" class="inputfile inputfile-6" accept="image/x-png,image/gif,image/jpeg">
        <label for="logo" class="commerce_logo"><span></span> <strong> Choose a file…</strong></label>
      </div>
      <?php 

        $thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($_GET['id']),'_thumbnail_id', true);
        if (!empty($thumbnail[0])) {
          echo '<img src="'.$thumbnail[0].'" class="logo_commerce_admin">';
        }
...
<div class="contact-inner-3 row">
    <div class="col-md-12 contact-inner-box-4 btn_submit_validiate">
     
      <input type="submit" class="btn" value="Submit" id="submit_validate" style="height:50px;">
    </div>
  </div>
  
  
  
    <div id="msg" style="margin:20px 0 0 0;"> </div>
