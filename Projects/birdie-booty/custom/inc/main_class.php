...
function feature_img($id=''){
	...
	$thumbnail = wp_get_attachment_image_src($id ,'full', false); 
	if (empty($thumbnail)) {
		$src = '';
	}else{
		$src = $thumbnail[0];
...
function get_size_id($id){
	global $obj, $wpdb;
	$data = $obj->getCol($wpdb->prefix.'bird_size', [
		'col' =>['name'],
		'where' =>['id' => $id],
		'return_type' => 'single'
	]);
...
function get_subscription_by_id($id){
	global $obj, $wpdb;
	$data = $obj->getCol($wpdb->prefix.'bird_subscriptions', [
		'col' =>['name'],
		'where' =>['id' => $id],
		'return_type' => 'single'
	]);
...
function get_subscription_amount_by_id($id){
	global $obj, $wpdb;
	$data = $obj->getCol($wpdb->prefix.'bird_subscriptions', [
		'col' =>['price'],
		'where' =>['id' => $id],
