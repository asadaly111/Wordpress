...
<?php if ($_SESSION['formdata']['serviceId'] != 41): $timess = json_decode($bokingobj->selectresourcepackages($_SESSION['formdata']['serviceId'])); ?>
...
<?php else: $data = json_decode($bokingobj->subservices($_SESSION['formdata']['serviceId'])); ?>
...
<div class="booking-sec">
	...
	<?php foreach ($data->subservices as $key): ?>
			...
			<div class="col-md-4 col-sm-12">
				<div class="booking-clm">
					<div class="user-duration">
						<h5>Select Duration<ruby><?php echo $key[0]->name; ?> </ruby></h5>
						<ul>
						<!-- 	<li>
								<label for="radio-1"><h6><h6>60 Minutes</h6> <p>£50   (personal training)</p></h6></label>
								<input type="radio" name="duration" value="60-minutes">
							</li> -->						
							<?php foreach ($key as $keykey): ?>
							<li class="undrli">
								<label for="radio-1"><h6>+<?php echo $keykey->slot; ?> Minutes<br></h6> <p> £<?php echo $keykey->price; ?></p></label>
								<input type="radio" name="duration" value="<?php echo $keykey->timeslot_id; ?>">
							</li>
							<?php endforeach; ?>
