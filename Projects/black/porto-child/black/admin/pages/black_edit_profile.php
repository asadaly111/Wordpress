...
@media (max-width: 1050px) 
{
	...
	.featured-box .box-content {padding: 30px 0px 10px 10px;}
	.proff-main form.profileimgchange {padding: 0 10px 0 0;}
	form.profileimgchange label i.fa-camera{right: 20px;}
	.profileimg{text-align: center;}
	.proff-main .proff .box.box-1{margin:0 5px 0 0;}
...
@media (max-width: 767px) 
{
	...
	form.profileimgchange label i.fa-camera {right: 0px;left: 0;text-align: center;margin: 0 auto;top: -30px;}
...
@media (max-width: 999px) and (min-width: 360px) 
{
...
<?php if ($dataservices): ?>
	<?php foreach ($dataservices as $key): $post_thumbnail_id = get_post_thumbnail_id( $key->ID ); $thumbnail = wp_get_attachment_image_src($post_thumbnail_id ,'thumbnail', false); 
		
		
		$black_age 	 = get_post_meta( $key->ID , 'black_age', true );
		$black_phone = get_post_meta( $key->ID , 'black_phone', true);
		$black_rates = get_post_meta( $key->ID , 'black_rates', true);
...
<!-- Start html here -->
<div class="proff-main">
	<div class="proff">
		<div class="row">
			<div class="col-md-3">
				<form action="" class="profileimgchange" enctype="multipart/form-data">
					<div class="profileimg">
						<img src="<?php echo $src; ?>" alt="" class="img-fluid profile">
						<!-- <a href="javascript:;" onclick="document.getElementById('upload').click()">click to change</a> -->
					</div>
			...
			<div class="col-md-4 pp">
				<label for="gender"><i class="fa fa-transgender"> </i>Age</label>
				<p><?php echo $black_age;?></p>
				<label for="company"><i class="fa fa-building"></i>Email</label>
				<p><?php echo $obj->get_user_email_by_id($key->post_author); ?></p>
				<div class="row">
					<div class="col-md-6">
						<div class="box box-1 text-center">
							<i class="fa fa-pencil"></i>
							<a href="?edit=true&id=<?php echo $key->ID;  ?>">Edit</a>
						</div>
					</div>
					<div class="col-md-6">
						<div class="box box-1">
							<i class="fa fa-picture-o" aria-hidden="true"></i>
							<a href="?gallery=true&id=<?php echo $key->ID;  ?>">Edit Gallery</a>
						</div>
					</div>
				</div>
			</div>
			<div class="col-md-5">
				<label for="joining date"><i class="fa fa-calendar"> </i> Created Date</label><span><a href="?edit=true&id=<?php echo $key->ID;  ?>"><i class="fa fa-edit"></i></a></span>
				<p>05-07-2018</p>
				<label for="Phone Number"><i class="fa fa-phone"></i>Phone Number</label>
				<p><?php echo $black_phone; ?></p>
				<div class="row">
					<div class="col-md-4">
						<div class="box box-1">
							<i class="fa  fa-video-camera"></i>
							<a href="?videos=true&id=<?php echo $key->ID;  ?>"> Add videos</a>
						</div>
					</div>
					<div class="col-md-4">
						<div class="box box-1">
							<i class="fa fa-eye" aria-hidden="true"></i>
							<a target="_blank" href="<?php echo get_the_permalink( $key->ID);  ?>">view profile</a>
						</div>
					</div>
					<div class="col-md-4">
						<div class="box box-1">
							<i class="fa fa-trash"></i>
							<a href="javascript:;" id="deactivate_ID" data-id="<?php echo $key->ID;  ?>">Delete Profile</a>
						</div>
					</div>
				</div>
			</div>
...
<div class="row edit-profile">
	...
	<div class="col-lg-12">
			Bio
			<textarea name="black_bio" class="form-control" rows="3" required><?php echo $post->post_content; ?></textarea>
	</div>
	<div class="col-lg-6">
			Phone Number
	...
	<div class="col-lg-6">
			Travel
		<select name="black_travel" class="form-control" required>
	...
	<div class="col-lg-6">
			Weight 
			<select name="black_weight" class="form-control" required>
	...
	<div class="col-lg-6">
		Ethnicity
		<select name="black_ethnicity" id="black_ethnicity" class="form-control" required>
	...
	<div class="col-lg-6">
			Smoker
		<select name="black_smoker" class="form-control" required>
	...
	<div class="col-lg-6">
			State
		<select name="black_state" class="form-control" required>
...
// deactivate profile #deactivate_ID
jQuery('#deactivate_ID').click(function(event) {
	var t = jQuery(this).data('id');
	jQuery.post('<?php echo site_url(''); ?>/wp-admin/admin-ajax.php?action=black_deactivate_account', {post_id: t }, function(data, textStatus, xhr) {
		swal({
			title: "Good job!",
			text: "Your profile has been deactivated now!",
			icon: "success",
			button: "ok",
		});
	});
...
jQuery(document).on('submit', '.profileimgchange', function(event) {
	event.preventDefault();
	jQuery(this).append('<div class="porto-ajax-loading"></div>');
	var form = jQuery(this);
	var formData = new FormData(jQuery(this)[0]);
	jQuery.ajax({
		type: 'post',
		url: '<?php echo site_url(''); ?>/wp-admin/admin-ajax.php?action=black_profile_picture',
		dataType: 'json',
		contentType: false,
		processData: false,
		data: formData,
	...
	.done(function(value) {
		jQuery('.porto-ajax-loading').remove();
		form.closest('.col-md-3').find('.profileimg img').attr('src', value.url);
