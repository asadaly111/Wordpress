...
function get_the_unready_message_count_admin($userid=0, $postid=0){
	global $obj, $wpdb;
	$count = $obj->getRows($wpdb->prefix.'messages', ['where' =>['sender_id' =>$userid, 'post_id' => $postid , 'adminstatus' => 'unseen' ] ]);
	if ($count) {
		return count($count);
	}else{
		return 0;
