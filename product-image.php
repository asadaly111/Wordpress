	...
	<div class="iosSlider product-slider">
		<div class="slider">
			<?php if (has_post_thumbnail()) {
				...
				$thumbnail 	= wp_get_attachment_image_src(get_post_thumbnail_id(),'shop_single', true);
			?>
			...
			if ( $product_attach_ids ) {
				foreach ($product_attach_ids as $product_attach_id) {
					...
					if (!$img_link)
						continue;
					$thumbnail1 	= wp_get_attachment_image_src($product_attach_id,'shop_single', true);
					printf( '<div class="slide"><div class="easyzoom"><a href="%s" title="%s"> %s </a></div></div>', $thumbnail1[0],esc_attr( get_post($product_attach_id)->post_title ), wp_get_attachment_image($product_attach_id, 'shop_single'));
